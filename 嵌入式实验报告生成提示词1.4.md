...
> **⚠️ python-docx 默认不设置列宽时，表格会根据内容自动收缩，导致表格宽度仅 4–5 cm，严重影响阅读体验。生成报告时必须显式设置每列的宽度，使表格总宽度接近页面可用宽度（约 14–16 cm）。**
...
# 设置两列总宽 ≈ 14 cm
col0_width = Cm(4.0)    # 标签列
col1_width = Cm(10.0)   # 内容列
...
```python
# 5 列表格推荐列宽分配（总宽 ≈ 14–15 cm）
widths = [Cm(1.2), Cm(3.0), Cm(4.0), Cm(1.2), Cm(4.6)]
...
def set_table_column_widths(table, widths: list):
    ...
    参数:
        ...
        widths: 各列宽度列表，如 [Cm(4.0), Cm(10.0)]
    """
    for row in table.rows:
        for idx, width in enumerate(widths):
