...
> 3. **节点内换行使用 `\n`**：`M2[处理步骤 1\n第二行说明]`，不要使用 `<br/>>` 标签（Kroki 不支持 HTML 标签）。
...
> **核心规则**：只有 `TOKEN_NEWLINE` 才触发段落切换。其余所有 token（包括 `TOKEN_WHITESPACE`）都在当前段落内添加为 run。
...
| 6 | 自动检测编码 | 依次尝试 UTF-8 → GBK → GB2312 | 硬编码 UTF-8，GBK 文件解码失败 |
...
> **每次生成报告前，请先回答以下问题。**
> 所有问题若未显式选择 [是] 或 [否]，**默认视为 [是]（自动生成）**。
> Q2/Q3/Q4 与 Q9/Q10/Q11 互斥：若对应 Q 选 [是]，则 Q2/Q3/Q4 的内容将被忽略。

### Q1：实验基本信息
- 实验次序（第几次实验）：______
- 实验项目名称：______
- 实验日期（如 2026.6.18）：______
- 实验地点（固定为 "竹园1129" 还是不同）：______
